...
export const LiTask = ({ key, nome, item, onclick, handleDelete, pasta }: LiTaskProps) => {
    return (
        <div key={key} className="bg-white items-center lg:w-full w-80 md:w-full shadow-md rounded-md p-2.5 border border-border lg:gap-4 gap-2 justify-between flex cursor-pointer">
            <li onClick={onclick} className="flex justify-between lg:w-full w-52 md:w-full items-center ">
            ...
            <div className="flex items-center gap-4">
                {pasta?.nome ? (
                    <div className="flex bg-gray-100 rounded-md text-center lg:w-32 w-20 pr-2 py-0.5 items-center justify-center">
                        <p className="lg:w-30 truncate lg:text-base w-16 text-sm">{pasta?.nome}</p>
                        <Image
                            src="/icon/folder.svg"
